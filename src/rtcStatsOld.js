...
class RTCStats {
  ...
  async logBatch(batchCollection) {
    ...
    let lastReport = this.previousReport;
    ...
    if (reportArray.length > 0) {
      this.previousReport = reportArray[reportArray.length - 1];

    }

    console.log("++++ [RTCStats] Previous report", lastReport)
    if (!reportArray.length) return;

    await this.store(reportArray);
    console.log("++++ [RTCStats] Stored raw data", reportArray);



    const transformedData = this.provider.transformStats(reportArray, lastReport);
