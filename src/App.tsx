...
export default function App() {
	...
	// Send settings hook - following the docs exactly
	const { updateSendSettings, sendSettings } = useSendSettings({
		onSendSettingsUpdated: logEvent,
	});

	// Use sendSettings in a useEffect to satisfy ESLint
	useEffect(() => {
		if (sendSettings) {
			console.log("Current send settings:", sendSettings);
		}
	}, [sendSettings]);

	// Update send settings function
	const updateVideoSendSettings = useCallback(() => {
		...
		updateSendSettings({
			...
			console.error("Error updating send settings:", err);
			logEvent({ action: "send-settings-error", error: err });
		});
	}, [callObject, updateSendSettings, logEvent]);

	// Send Settings component definition within App
	const SendSettings = () => {
		return (
			<div>
	...
	return (
		<>
			<div className="App">
				...
				<br />
				<button onClick={updateVideoSendSettings}>
					Update Video Send Settings
				</button>
			</div>
